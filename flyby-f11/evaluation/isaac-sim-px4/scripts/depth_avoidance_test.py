...
class InProcessDepthEstimator:
    ...
    def estimate_depth(
        ...
        try:
            ...
            # Step 3: Normalize relative depth to 0-1 range for scale corrector
            rel_min = relative_depth.min()
            rel_max = relative_depth.max()
            if rel_max > rel_min:
                relative_depth_norm = (relative_depth - rel_min) / (rel_max - rel_min)
            else:
                # Flat field - everything same depth
                relative_depth_norm = np.ones_like(relative_depth) * 0.5

            # Debug normalization on first frame
            if not hasattr(self, '_logged_norm'):
                self._logged_norm = True
                print(f"[DEBUG] relative_depth range=[{rel_min:.3f}, {rel_max:.3f}] -> "
                      f"normalized [0.0, 1.0] (higher=farther)", flush=True)

            # Step 4: Update scale corrector with rangefinder
            # scale = rangefinder_reading / center_depth_normalized
            # This gives us the conversion factor from 0-1 to meters
            scale_estimate = self.scale_corrector.update(
                relative_depth_norm, rangefinder_reading, rangefinder_valid
            )
            ...
            if self._frame_count % 30 == 0:
                ...
                center_val = self.scale_corrector._sample_center(relative_depth_norm)
            ...
            # Step 5: Apply scale to get metric depth
            # metric_depth = scale * relative_depth_norm
            metric_depth = self.scale_corrector.apply(relative_depth_norm, min_depth=0.5)
            ...
            # Return metric depth, metadata, and normalized relative depth for debugging
            return metric_depth, depth_data, relative_depth_norm
